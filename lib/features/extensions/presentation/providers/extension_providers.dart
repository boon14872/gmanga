...
final extensionRepositoryProvider = FutureProvider<ExtensionRepository>((
  ref,
) async {
...
final extensionListProvider =
    StateNotifierProvider<
      ExtensionListNotifier,
      AsyncValue<List<ExtensionSource>>
    >((ref) {
      return ExtensionListNotifier(ref);
    });

class ExtensionListNotifier
    extends StateNotifier<AsyncValue<List<ExtensionSource>>> {
  final Ref _ref;
  ...
  Future<void> _loadExtensions() async {
    ...
    try {
      final repositoryAsync = await _ref.read(
        extensionRepositoryProvider.future,
      );
  ...
  Future<ExtensionSource?> loadExtensionFromFile() async {
    try {
      ...
      if (result != null && result.files.single.bytes != null) {
        ...
        final extensionSource = _parseExtensionFromScript(
          fileContent,
          fileName,
        );
  ...
  ExtensionSource? _parseExtensionFromScript(String content, String fileName) {
    try {
      ...
      final nameMatch = RegExp(
        r'name:\s*["\x27]([^"\x27]+)["\x27]',
      ).firstMatch(content);
      final versionMatch = RegExp(
        r'version:\s*["\x27]([^"\x27]+)["\x27]',
      ).firstMatch(content);
      final langMatch = RegExp(
        r'lang:\s*["\x27]([^"\x27]+)["\x27]',
      ).firstMatch(content);
